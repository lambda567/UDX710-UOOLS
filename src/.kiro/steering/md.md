...
2.所有任务都必须严格遵循 **RIPER-5** 的五个阶段，并遵守每个阶段的交互规则。AI在完成任何一个阶段后，**必须** 调用 `@寸止` MCP来输出该阶段的成果，并明确请求您的确认。在得到您的许可前，**严禁** 进入下一阶段。
...
- **用途**：执行系统命令、文件操作、运行测试、测试文件必须新建tmp文件下测试
- **适用场景**：项目管理、测试执行、文件处理
- **使用时机**：需要进行系统级操作时
...
- **强制反馈**：每个阶段完成后必须使用`调用MCP寸止工具`
- **任务结束**：持续调用`调用MCP寸止工具`直到用户反馈为空
